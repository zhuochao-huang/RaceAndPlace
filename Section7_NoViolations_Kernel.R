...
for (ni in 1 : nSim) {
  ...
  P_Y0<- sigmoid(X-0.5 - 2*R_value)
  P_Y1<- sigmoid(X-0.5 - 2*R_value)
  for (i in 1:n) {
    Y_D0[i]<-rbinom(1,1,P_Y0[i])
    Y_D1[i]<-rbinom(1,1,P_Y1[i])
    if (D[i]==0){Y_full[i]=Y_D0[i]}
    else{Y_full[i]=Y_D1[i]}
    if (M[i]==0){Y[i]=0}
    else{Y[i] <- Y_full[i]}
  }
  ...
  Y_RX<- glm(Y_D1~ R_value + X_factor,family = "binomial")
  ...
  P_X_MR0<-as.numeric(predict(X_MR, newdata = newdataR0, "probs")["3"])
  R_M1X<-lm(R_value_M1~X_factor_M1)
  P_X_M1<-sum(X==3&M==1)/sum(M==1)
  out3 <- as.numeric(P_X_MR*  R_M1_kde(r)/ P_X_M1)
  out3R0 <- as.numeric(P_X_MR0*  R_M1_kde(0.6)/ P_X_M1)
...
p1<-ggplot()+
  ...
  xlim(0.2,0.8) +
  labs(x = "r value", y = "Race and Place Estimand",title = "No Violation of Assumption")+
