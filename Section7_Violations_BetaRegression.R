...
simData = function(n, n_precinct) {
  ...
  ## Generate unmeasured variable U
  u<-runif(n, min = 0, max = 1)
  
  ## Generate D variable
  P_D <- expit(u+2*p/n_precinct-1+rnorm(n))
  D <- rbinom(n,1,P_D)
  ...
  P_Y01<- (u)*expit(X-0.5 - 2*R_value)
  P_Y11<- (u)*expit(X-0.5 - 2*R_value)
...
estPsiFunction = function(r_seq, x, r0, Y, D, R_value, M, X_factor) {
  ...
  term1 = term2 = term3 = term4 =term33=term44= matrix(NA, length(x), length(r_seq))
  ...
  for (ii in 1 : length(r_seq)) {
    term33[,ii] = betaDensity(r = r_seq[ii], mu = mu3, phi = phi3)
  }
  ...
  for (ii in 1 : length(r_seq)) {
    term44[,ii] = betaDensity(r = r_seq[ii], mu = mu4, phi = phi4)   
  }
  
  ## Final estimated curves
